...
var pool = module.exports = function pool(options, authorizeFn) {
    ...
    function StartStratumServer(finishedCallback) {
        ...
        }).on('refuse', function(ip, message, sendOther){
    ...
    function SetupBlockPolling() {
        ...
        if (typeof options.subsidyRefreshInterval !== "number" || options.subsidyRefreshInterval <= 0) {
            emitLog('Block Subsidy polling has been disabled');
            return;
        }
		
        var pollingInterval = options.blockRefreshInterval;
		var subsidyInterval = options.subsidyRefreshInterval;
		
        subsidyIntervalId = setInterval(function () {
            GetBlockSubsidy(function (error, result) {});
        }, subsidyInterval);
		...
		_this.daemon.cmd(
			...
			function (result){
				if (result[0].error){
					...
					subsidy = result[0].response;
					callback(result[0].error, result[0].response);
				}				
			}
        )		
	}
    ...
    function GetBlockTemplate(callback) {
		...
		if (!subsidy) {
			GetBlockSubsidy(function(error, reuslt){
				if(error){
					callback(error);
				}
				else{
